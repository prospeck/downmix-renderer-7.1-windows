...
class AudioEngineSettingsTests(unittest.TestCase):
    ...
    def test_python_callback_updates_liveness_counters(self) -> None:
        ...
        self.assertEqual(snapshot.xrun_count, 0)
        self.assertFalse(snapshot.mmcss_registered)
        self.assertTrue(np.all(outdata == 0.0))

    def test_python_callback_status_updates_xrun_health_counter(self) -> None:
        class FakeProcessor:
            def set_master_volume(self, scalar: float, muted: bool = False) -> None:
                return None
        ...
        engine._callback(indata, outdata, 128, None, "output underflow")
        snapshot = engine.snapshot()
        engine.close()

        self.assertEqual(snapshot.callback_status, "output underflow")
        self.assertEqual(snapshot.callback_status_count, 1)
        self.assertEqual(snapshot.xrun_count, 1)
        self.assertTrue(np.all(outdata == 0.0))
